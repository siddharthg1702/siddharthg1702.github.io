...
if($type == "onsite")	{
	...
	if($w_name == "Arduino Essentials")
	{
		// echo "arduiE";
		if($tickets != "t1")	{
		$remaining = $tickets*700;
		}
		$remaining = $remaining + $kits*800 - $total;
	}
...
$surl = "http://dexbot.mitindia.edu/main/success.php?name=$name&email=$email&txnid=$txnid";
$furl = "http://dexbot.mitindia.edu/main/failure.php?txnid=$txnid";	
date_default_timezone_set('Asia/Kolkata');
$time = (string)date("h:i:sa");
$date = (string) date("d-m-Y");
$datetime = $date.' '.$time;
$qry1="insert into dexbot(name,email,phone,college,year,tickets,txnid,pid,amount,kits,type,workshop,remaining,date)
VALUES('$name','$email','$phone','$college','$year','$tickets','$txnid','$pid','$total','$kits','$type','$w_name','$remaining','$datetime')";
